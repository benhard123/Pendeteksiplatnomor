...
def main():
    ...
    for npaContour in npaContours:                          # for each contour
        if cv2.contourArea(npaContour) > MIN_CONTOUR_AREA:          # if contour is big enough to consider
            ...
            resizedimage = cv2.resize(imgTrainingNumbers, (int(imgTrainingNumbers.shape[1]*scale),int(imgTrainingNumbers.shape[0]*scale)))
            cv2.imshow("training_numbers.png", resizedimage)      # show training numbers image, this will now have red rectangles drawn on it
